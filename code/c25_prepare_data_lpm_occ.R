...
df_lpm_data_occ <- df_merged_2m_all

rm(df_merged_2m_all)

# dataset for linear probability model - restricted sample
df_lpm_data_occ <-
    df_lpm_data_occ %>%
    ...
    unite(status_1, lfs_1, occ1cat_1, sep = "_") %>%
    unite(status_2, lfs_2, occ1cat_2, sep = "_") %>%
    ...
    mutate(# dependent variables
           y_E_RM  = (status_2 == "E_RM") %>% as.integer(),
           y_E_RC  = (status_2 == "E_RC") %>% as.integer(),
           y_E_NRM = (status_2 == "E_NRM") %>% as.integer(),
           y_E_NRC = (status_2 == "E_NRC") %>% as.integer(),
           y_U_RM  = (status_2 == "U_RM") %>% as.integer(),
           y_U_RC  = (status_2 == "U_RC") %>% as.integer(),
           y_U_NRM = (status_2 == "U_NRM") %>% as.integer(),
           y_U_NRC = (status_2 == "U_NRC") %>% as.integer(),
           y_I     = (status_2 == "I_X") %>% as.integer(),
           # age categories
           ...
           cycle_id = case_when(period_1 %in% 197601:197912 ~ "E1",
                                ...
                                TRUE                        ~ NA_character_)) %>%
    select(-c(gestcen, age_1, educ_1, female, white, udur_1, status_2))

df_lpm_data_occ <-
    bind_rows(df_lpm_data_occ,
              df_lpm_data_occ %>%
...
save(df_lpm_data_occ, file = str_c(edir_cps, "out_lpm_data_occ.Rdata"))
