...
df.state.codes <-
    read_csv(paste0(mdir.cps, "data/state_codes.csv"), col_types = c("iicc")) %>%
...
print(paste("Extracting relevant data from CPS", tfirst, "to", tlast))
...
while (t <= tlast) {
    ...
    if (t <= 197712) {
        df.cpsdata <-
            read_fwf(file = paste0(ddir.cps, "b", t),
    ...
    if (t >= 199401) {
       ...
       if ((t >= 200001) && (t <= 200212)) {
          ...
          df.cpsdata.revised <-
              read_fwf(file = paste0(ddir.cps, "revised/b", t, "revised"),
          ...
          df.cpsdata <- inner_join(df.cpsdata.origin, df.cpsdata.revised, by=c("QSTNUM","OCCURNUM"))
       }
        ...
        df.cpsdata %<>%
            mutate(period = t,
                   ...
                   HRHHID2 = paste0(HRSAMPLE, HRSERSUF, gsub("0", "", HUHHNUM)),
                   HID = paste0(sprintf("%02d", GESTFIPS), HRHHID, HRHHID2)) %>%
    ...
    if (t <= 198812) df.cpsdata %<>% mutate(HGA = HGA - 1L)
    # subtract one year if the highest grade attended is not completed
    if (t <= 199112) df.cpsdata %<>% mutate(PEEDUCA = if_else(!is.na(HGC) & HGC == 2, HGA - 1L, HGA))

    # add GESTCEN
    if (t >= 201401) df.cpsdata %<>% left_join(df.state.codes, by = "GESTFIPS")
    ...
    df.cpsdata %<>%
        mutate(period = t,
               ...
               occ1cat.all = case_when(is.na(PEIO1OCD) ~ "X",
                                        ...
                                        (period <= 198212) & (PEIO1OCD %in% occ.grp$Y1970$RC)  ~ "RC",
                                        (period <= 198212) & (PEIO1OCD %in% occ.grp$Y1970$RM)  ~ "RM",
                                        (period <= 198212) & (PEIO1OCD %in% occ.grp$Y1970$FRM) ~ "FRM",
                                        (period <= 198212) & (PEIO1OCD %in% occ.grp$Y1970$MIL) ~ "MIL",

                                        (period >= 198301 & period <= 200212) & (PEIO1OCD %in% occ.grp$Y1980$NRC) ~ "NRC",
                                        (period >= 198301 & period <= 200212) & (PEIO1OCD %in% occ.grp$Y1980$NRM) ~ "NRM",
                                        (period >= 198301 & period <= 200212) & (PEIO1OCD %in% occ.grp$Y1980$RC)  ~ "RC",
                                        (period >= 198301 & period <= 200212) & (PEIO1OCD %in% occ.grp$Y1980$RM)  ~ "RM",
                                        (period >= 198301 & period <= 200212) & (PEIO1OCD %in% occ.grp$Y1980$FRM) ~ "FRM",
                                        (period >= 198301 & period <= 200212) & (PEIO1OCD %in% occ.grp$Y1980$MIL) ~ "MIL",

                                        (period >= 200301 & period <= 201012) & (PEIO1OCD %in% occ.grp$Y2000$NRC) ~ "NRC",
                                        (period >= 200301 & period <= 201012) & (PEIO1OCD %in% occ.grp$Y2000$NRM) ~ "NRM",
                                        (period >= 200301 & period <= 201012) & (PEIO1OCD %in% occ.grp$Y2000$RC)  ~ "RC",
                                        (period >= 200301 & period <= 201012) & (PEIO1OCD %in% occ.grp$Y2000$RM)  ~ "RM",
                                        (period >= 200301 & period <= 201012) & (PEIO1OCD %in% occ.grp$Y2000$FRM) ~ "FRM",
                                        (period >= 200301 & period <= 201012) & (PEIO1OCD %in% occ.grp$Y2000$MIL) ~ "MIL",

                                        (period >= 201101) & (PEIO1OCD %in% occ.grp$Y2010$NRC) ~ "NRC",
                                        (period >= 201101) & (PEIO1OCD %in% occ.grp$Y2010$NRM) ~ "NRM",
                                        (period >= 201101) & (PEIO1OCD %in% occ.grp$Y2010$RC)  ~ "RC",
                                        (period >= 201101) & (PEIO1OCD %in% occ.grp$Y2010$RM)  ~ "RM",
                                        (period >= 201101) & (PEIO1OCD %in% occ.grp$Y2010$FRM) ~ "FRM",
                                        (period >= 201101) & (PEIO1OCD %in% occ.grp$Y2010$MIL) ~ "MIL",
    ...
    # weights
    if (t <= 199712) df.cpsdata %<>% mutate(weight = PWSSWGT)
    if (t >= 199801) df.cpsdata %<>% mutate(weight = PWCMPWGT)
    if (t >= 200001 & t <= 200212) df.cpsdata %<>% mutate(weight = NWCMPWGT)
